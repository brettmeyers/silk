...
it is read.  When the B<--xargs> switch is provided, B<rwaddrcount>
...
IP addresses will be printed.  When this switch is not specified, IPs
are printed in the canonical format.  The I<FORMAT> is one of:
...
B<--output-path> switch has been used to redirect B<rwaddrcount>'s
ASCII output.

=item B<--output-path>=I<PATH>

Determine where the output of B<rwaddrcount> (ASCII text) is written.
...
(new) style, C<I<YYYY>/I<MM>/I<DD>TI<hh>:I<mm>:I<ss>>, or the legacy
style, S<C<I<MM>/I<DD>/I<YYYY> I<hh>:I<mm>:I<ss>>>.  When this switch
...
Causes B<rwaddrcount> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwaddrcount> will open each file in turn
...
the output from B<rwaddrcount --version>), the value of the TZ
environment variable determines the timezone in which B<rwaddrcount>
displays timestamps.  If the TZ environment variable is not set, the
...
$SiLK: rwaddrcount.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
