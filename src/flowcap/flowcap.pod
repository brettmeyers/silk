...
B<flowcap> - Capture, temporarily store, and forward flow data
...
B<flowcap> is a daemon that collects data from devices that produce
flow data (such as a router producing NetFlow v5 or a flow meter
producing IPFIX (Internet Protocol Flow Information eXport)),
temporarily stores the data to files on its local disk, and forwards
these files to B<rwflowpack(8)> for packing.
...
B<rwsender>.  This switch is required.

=item B<--sensor-configuration>=I<FILENAME>

Give the path to the configuration file that B<flowcap> will consult
...
If a B<flowcap> file's exceeds I<SIZE> bytes, it will be closed, and a
new file will be created and used.  In addition, before opening an
output file, B<flowcap> ensures there are I<SIZE> bytes of free space
...
first duration will be less than I<TIMEOUT> seconds.  If the
...
hour.  Even with this switch, files will still be closed if they
...
fills this space, it will exit.  Flows arriving during this time will
be lost.  See also B<--space-maximum-percent>.
...
the file system.  If B<flowcap> fills this space, it will exit.  See
also B<--freespace-minimum>.
...
B<flowcap> exits with an error.  Specifying this switch without also
...
B<silk(7)>, B<syslog(3)>, B<zlib(3)>, B<gzip(1)>,
I<SiLK Installation Handbook>

=cut

$SiLK: flowcap.pod 451cb3934c03 2014-03-07 20:44:02Z mthomas $
