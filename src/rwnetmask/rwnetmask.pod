...
it is read.  When the B<--xargs> switch is provided, B<rwnetmask> will
...
Write the output to the named I<PATH>.  I<PATH> may be a file, named
pipe, or the symbols C<stdout> or C<-> to write to the standard
output.  When not specified, output will be written to the standard
output.  B<rwnetmask> will exit with an error if the output path is
...
Causes B<rwnetmask> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwnetmask> will open each file in turn and
...
$SiLK: rwnetmask.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
