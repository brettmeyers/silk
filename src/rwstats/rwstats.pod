...
B<rwstats> will read the names of the files to process from the named
...
Flow records are read, the bins are sorted by the primary aggregate
value, and B<rwstats> prints the bins that had the largest values
(giving a top-N) list or the smallest values (giving a bottom-N list).
The number of bins printed can be specified as a fixed value (e.g.,
print 10 bins), as a threshold (print bins whose byte count is less
than 400), or as a percentage of the total volume across all bins
(print bins who that contain at least 10% of all the packets).
...
are specified and B<--presorted-input> is given, B<rwstats> will
merge-sort the flow records from the input files.  B<rwstats> will
usually run faster if you do I<not> include the B<--presorted-input>
...
B<rwstats> may run out of memory when computing distinct IP counts,
causing the counts for some bins to be smaller than the actual number
of distinct IPs.  When this occurs, a single warning is printed the
standard error noting that B<rwstats> has run out of memory, processing
continues, and B<rwstats> exits with status 16.
...
field(s), but for backward compatibility the B<--fields> switch is not
required.
...
fields C<sTime>,C<duration>,C<eTime> are all in use, B<rwstats> will ignore
...
but not C<type>, B<rwstats>'s output will have multiple rows with the
...
label determined by passing the source IP or the protocol/source-port
...
field(s) will be binned together.  For each bin, one or more aggregate
...
C<Distinct:sPort> will count the number of
...
page.  When I<PLUGIN> does not contain a slash (C</>), B<rwstats> will
...
Instruct B<rwstats> to load the mapping file located at I<PATH> and
...
specific to that prefix map.  If I<MAPNAME> is not provided,
B<rwstats> will check the prefix map file to see if a map-name was
specified when the file was created.  If no map-name is available,
B<rwstats> creates the fields sval and dval.  Multiple B<--pmap-file>
switches are supported as long as each uses a unique value for
map-name.  The B<--pmap-file> switch(es) must precede the B<--fields>
switch.  For more information, see B<pmapfilter(3)>.
...
Print the I<N> bins with the largest (or smallest) values.  This limit
is always allowed.

=item B<--threshold>=I<N>

Print the bins where the primary value is greater-than (or less-than)
the value I<N>.  This limit is not allowed when the primary value
comes from a plug-in.  If the threshold causes the Top-N or Bottom-N
to become large enough that B<rwstats> runs out of memory, B<rwstats>
will compute the Top-N or Bottom-N using the amount of memory it was
able to allocate.

=item B<--percentage>=I<N>

Print the bins where the primary value is greater-than (or less-than)
I<N> percent of the sum of the primary values across all bins.  To use
this switch, the primary value must be C<Bytes>, C<Packets>, or
C<Records>, and the B<--presorted-input> switch must not be present.
If the percentage causes the Top-N or Bottom-N to become large enough
that B<rwstats> runs out of memory, B<rwstats> will compute the Top-N
or Bottom-N using the amount of memory it was able to allocate.
...
I<exact same> value for the B<--fields> switch.  This option allows
B<rwstats> to process an endless stream of records.  When multiple
input files are specified, B<rwstats> will merge-sort the flow records
from the input files.  When using B<--presorted-input> and computing a
Top-N or Bottom-N, the B<--percentage> limit cannot be used.  See the
L</NOTES> section for issues that may occur when using
B<--presorted-input>.

=item B<--no-percents>

For the Top-N invocation, do not print the percent-of-total and
cumulative-percentage columns.  These columns will contain a question
mark when the primary key is not one of C<Bytes>, C<Packets>, or
C<Records>, and this switch allows you to suppress them.
...
B<--output-path> switch has been used to redirect B<rwstats>'s ASCII
output.

=item B<--output-path>=I<PATH>

Determine where the output of B<rwstats> (ASCII text) is written.
If this option is not given, output is written to the standard output.
...
Causes B<rwstats> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwstats> will open each file in turn and
...
key.  Using this switch with IPv6 data will cause an error.  The user
...
the key.  Using this switch with IPv6 data will cause an error.  The
...
B<--ipv6-policy> when that switch is not provided.
...
B<--python-file> is specified, B<rwstats> loads Python which in turn
...
the output from B<rwstats --version>), the value of the TZ environment
variable determines the timezone in which B<rwstats> displays
timestamps.  If the TZ environment variable is not set, the default
timezone is used.  Setting TZ to 0 or the empty string causes
timestamps to be displayed in UTC.  The value of the TZ environment
...
all in use, B<rwstats> will ignore the final time field when binning
the records, but the field will appear in the output.  Due to
truncation of the milliseconds values, B<rwstats> will generate
different numbers of bins depending on the order in which those three
...
B<rwstats> truncates the millisecond value.

When computing distinct IP counts, B<rwstats> will typically run
faster if you do I<not> use the B<--presorted-input> switch, even if
the data was previously sorted.

B<rwstats> may run out of memory when computing distinct IP counts,
causing the counts for some bins to be smaller than the actual number
of distinct IPs.  When this occurs, a single warning is printed the
standard error noting that B<rwstats> has run out of memory,
processing continues, and B<rwstats> exits with status 16.
...
B<rwflowpack(8)>, B<silk(7)>, B<yaf(1)>, B<dlopen(3)>

=cut

$SiLK: rwstats.pod 477e54df24bf 2014-07-08 18:02:17Z mthomas $
