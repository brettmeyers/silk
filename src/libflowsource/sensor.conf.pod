...
the external library libfixbuf, version 1.6.0 or later.  I<Since SiLK
3.9.0.>
...
When listening to IPFIX data from B<yaf>, this is the value specified
to B<yaf>'s B<--ipfix-port> switch.
...
This optional command specifies the IPs or names of the hosts that are
...
B<flowcap>.  (Prior to SiLK 3.10.1, the B<accept-from-host> command
accepted only a single argument.)
...
with a dot (C<.>) are ignored.  This command may be used with the
following probe types:
...
=item B<log-flags> { B<none> | { B<all> | B<bad> | B<default> | B<firewall-event> | B<missing> | B<record-timestamps> | B<sampling> | ... } }
...
Requires libfixbuf-1.4.0 or later.  I<Since SiLK 3.8.0>.
...
used in sensor blocks as described in the descriptions for the
B<discard-when>, B<discard-unless>, I<network-name>B<-ipblocks>,
I<network-name>B<-ipsets> and I<network-name>B<-interfaces>
commands, below.
...
I<group reference> is the name of the group prefixed by the at (C<@>)
character.
...
=item B<interfaces> {I<integer> | I<group-ref>} [I<integer> I<group-ref>...]
...
The B<interfaces> command may appear multiple times in a group block.
Each integer value may be between 0 and 65535 inclusive.

=item B<ipblocks> {I<cidr-block> | I<group-ref>} [I<cidr-block> I<group-ref>...]

The B<ipblocks> command adds CIDR block values to a group.  The
B<ipblocks> command may appear multiple times in a group block.  For
groups containing more than a couple of CIDR blocks, consider using an
IPset instead.

=item B<ipsets> {I<filename> | I<group-ref>} [I<filename> I<group-ref>...]

The B<ipset> command adds the IP addresses specified in an IPset file
(such as that created by B<rwsetbuild(1)>) a group.  The B<ipsets>
command may appear multiple times in a group block.  When multiple
IPset files are specified, the group maintains a single IPset that is
the union of files.  B<rwflowpack> exits with an error if the IPset
file does not exist or does not contain any IP addresses.  I<Since
SiLK 3.10.0>.

=back

Specifying the at (C<@>) character and the name of an existing group
within an B<interfaces>, B<ipblocks>, or B<ipsets> command causes the
contents of the existing group to be added to the current group as
long as the existing group contains interfaces, ipblocks, or ipsets,
respectively.  A group does not reference other groups; the contents
of the existing groups are copied into the current group.

For example group blocks, see L</Group definitions> below.
...
reference to a group (see L</Group Block>) containing ipblocks,
ipsets, or interfaces, respectively.  (A group reference the at (C<@>)
character followed by group's name.)  These lists are defined as
follows:
...
A cidr-block-list contains one or more CIDR blocks or group references
that represent an address space.  As part of determining how to
process a flow record, B<rwflowpack> may check whether the record's
source or destination IP address is in the list.  When comparing an IP
address to a cidr-block-list, note the following:
...
An ipset-list contains the names of one or more IPset files or group
references.  These files represent an address space, and B<rwflowpack>
may check whether a flow record's source or destination IP address is
in this address space.  When multiple IPset files are specified, the
contents of the files are merged into a single IPset.  B<rwflowpack>
exits with an error if the IPset file does not exist or does not
contain any IP addresses.  The rules for comparing IPv4 and IPv6
addresses are the same as those for the cidr-block-list.  I<Since SiLK
3.10.0>.

=item I<interface-list>

An interface-list contains one or more group references or integers
(ranging from 0 to 65535) that represent SNMP interface index(es) or VLAN
...
B<remainder>, the IP-space for I<network-name> is conceptually all IPs
not assigned to other networks on this sensor.  The B<remainder>
...
This command specifies the IP-space that is assigned to the network
named I<network-name>.  The value of the command can be the keyword
B<remainder> or a list containing the names of IPset files and/or
group references to groups containing IPset files.  When the value is
...
containing interfaces.  When the value is the keyword B<remainder>,
the interface list is computed by finding all interface values not
assigned to other networks on this sensor.  The B<remainder> keyword
...
$SiLK: sensor.conf.pod 68603dc525b7 2015-02-17 15:50:48Z mthomas $
