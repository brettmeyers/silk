...
A group is a named list that contains either CIDR blocks or integers.
The use of groups is optional; the primary purpose of a group is to
allow the administrator to specify a commonly used list (such as the
IP space of the network being monitored) in a single location.  Groups
were introduced in SiLK 2.1.
...
are accepted within the probe context.  Note that one and only one of
B<listen-on-port>, B<listen-on-unix-socket>, B<read-from-file>, or
B<poll-directory> must be specified.
...
libfixbuf, version 1.3.0 or later.
...
the external library libfixbuf, version 1.6.0 or later.  I<Available
since SiLK 3.9.0.>
...
This command specifies the network port where the probe should collect
flow data.  When listening to NetFlow from a Cisco router, this is the
I<port> that was specified to the Cisco ISO command
...
=item B<accept-from-host> I<host-name>

This optional command specifies the IP or name of the host that is
allowed to connect to the port where the probe is listening.  When
this command is not present, any host may connect.  The command may
only be specified when the B<listen-on-port> command is also present.
When listening for NetFlow, this attribute would be the IP address of
the router as seen from the machine running B<rwflowpack> or
B<flowcap>.

This paragraph applies only when using versions of SiLK and libfixbuf
prior to 3.4.0 and 1.2.0, respectively: The B<accept-from-host>
command cannot be used with UDP-based IPFIX probes or NetFlow v9
probes.  If multiple probes of these types are needed, they should
have different B<listen-on-port> values.
...
I<path-to-unix-socket> must exist.
...
since B<flowcap> does not support reading data from files.  When
polling the directory, zero length files and files whose name begin
with a dot (C<.>) are ignored.  This command may be with the following
probe types:
...
=item B<log-flags> { B<none> | B<all> | { B<bad> | B<missing> | B<sampling> | B<firewall-event> ... } }
...
the B<log-flag> command to adjust the information logged.  The
possible values are:
...
Write messages about bad NetFlow v5 packets.  An example of a bad
packet is a NetFlow v5 PDU that reports no flow records.

=item I<missing>

Examine the sequence numbers of NetFlow v5 packets and write messages
about missing packets.  The netflow-v5 probe keeps track of the
sequence numbers in the PDUs it processes (whether read from the
network or from a file).  By default, when the probe determines that
PDUs are missing or invalid, it will write a message to the log; note
that an out-of-order UDP packet will also cause a message about
missing records.
...
When the C<firewall-event> quirks flag is set, write messages about
records that are ignored because the firewall event is something other
than flow deleted or flow denied.  I<Since SiLK 3.8.1>.
...
=item B<quirks> { B<none> | { B<firewall-event> | B<zero-packets> | B<missing-ips> ... } }
...
B<none>, it may be a list of one or more of the following values:
...
processing IPFIX or NetFlow v9 flow records.  The IPFIX firewallEvent
...
messages, use the B<log-flags> statement.)  When this quirk is not
...
records that contained a firewall event IE.)
...
to support large byte-per-packet ratios.
...
a list of commonly used CIDR blocks or integer values that are treated
as SNMP interfaces or VLAN identifiers.  Groups are used in sensor
blocks as described in the descriptions for the B<discard-when>,
B<discard-unless>, I<network-name>B<-ipblocks>, and
I<network-name>B<-interfaces> statements, below.

The statements in a group definition must all be of the same type.
That is, you cannot mix B<interfaces> and B<ipblocks> statements in a
...
=item B<interfaces> I<integer-or-group-ref> [I<integer-or-group-ref>...]

Interfaces values are added to a group by means of the B<interfaces>
command within a group block.  Existing interface groups may be added
by prefixing each group's name with an at (C<@>) character.  When you
add an interface group, it is the same as individually adding each
interface to the group being defined.  It is an error to add an
ipblock group to an interface group.  The B<interfaces> command may
appear multiple times in a group block.

=item B<ipblocks> I<cidr-block-or-group-ref> [I<cidr-block-or-group-ref>...]

CIDR block values are added to a group by means of the B<ipblocks>
command within a group block.  Existing ipblock groups may be added by
prefixing each group's name with an at (C<@>) character.  When you add
an ipblock group, it is the same as individually adding each CIDR
block to the group being defined.  It is an error to add an interface
group to an ipblock group.  The B<ipblocks> command may appear
multiple times in a group block.
...
list of CIDR blocks or a list of integers.  These are defined as
follows:
...
A cidr-block-list contains one or more CIDR blocks that represent an
address space.  As part of determining how to process a flow record,
B<rwflowpack> may check whether the record's source or destination IP
address is in the list.  The cidr-block-list can be created by
specifying CIDR blocks and/or references to groups (see L</Group
Block>) containing CIDR blocks (ipblocks).  To add a reference to a
group, prefix the group's name with an at (C<@>) character.  It is an
error to add an interface group to a cidr-block-list.  When comparing
an IP address to a cidr-block-list, note the following:

=over 4

=item *

the IP address is compared to each element in the cidr-block-list,
stopping once a match is made

=item *

when comparing an IPv4 address to a cidr-block-list element that is
IPv6, the IPv4 address is converted to IPv6 by mapping it into the
::ffff:0:0/96 prefix for purposes of the comparison

=item *

when comparing an IPv6 address to a cidr-block-list element that is
IPv4, an IPv6 address in the ::ffff:0:0/96 prefix will be converted to
IPv4 for purposes of the comparison and any other IPv6 address will
fail the comparison
...
An interface-list contains one or more non-negative integers ranging
from 0 to 65535 that represent SNMP interface index(es) or VLAN
identifiers.  As part of determining how to process a flow record,
B<rwflowpack> may check whether the record's C<input> or C<output>
fields are in the list.  Whether the C<input> and C<output> fields
contain SNMP interfaces or VLAN identifiers is determined by the
B<interface-values> command in the probe block (c.f. L</Probe Block>).
The interface-list can be creating by specifying numbers and/or
references to groups containing interfaces.  To add a reference to a
group, prefix the group's name with an at (C<@>) character.  It is an
error to add an ipblock group to an interface-list.
...
This optional command instructs B<rwflowpack> to discard a flow record
when the value in the C<input> field is listed in I<interface-list>.
When a match is found, B<rwflowpack> immediately stops processing that
record and it will not pack the flow for this sensor.  When this
command is present, no other B<discard-* source-interfaces> commands
may be specified on this sensor, but you may specify a B<discard-*
source-ipblocks> statement.  When paired with VLAN tagging (see the
B<interface-values> statement in the probe block), this allows the
administrator to discard flows that have a specific VLAN tag.

=item B<discard-when> B<destination>-B<interfaces> I<interface-list>

As above, but discards a flow when the value in the record's C<output>
field matches a value in I<interface-list>.

=item B<discard-when> B<any>-B<interfaces> I<interface-list>

As above, but discards a flow when either its C<input> or C<output>
field matches a value in I<interface-list>.
...
These optional commands are similar to the
S<B<discard-when> I<*>-B<interfaces>> commands above except they
operate on IP addresses.  That is, the first discards a flow when its
source IP address is in I<cidr-block-list>, the second ignores a flow
when its destination IP address is in I<cidr-block-list>, and the
third discards a flow when either its source IP or destination IP
address is in I<cidr-block-list>.

=item B<discard-unless> B<source>-B<interfaces> I<interface-list>

This optional command is similar to the B<discard-when> command
described above, except this command instructs B<rwflowpack> to
discard the flow record I<unless> its C<input> field is in
I<interface-list>.  That is, the flow record will be packed only when
all the B<discard-unless> statements are matched.  When this command
is present, no other B<discard-* source-interfaces> commands may be
specified on this sensor, but you may specify a B<discard-*
source-ipblocks> statement.  When paired with VLAN tagging (see the
B<interface-values> statement in the probe block), this allows one to
discard flows that do not have a specific VLAN tag.

=item B<discard-unless> B<destination>-B<interfaces> I<interface-list>

As above, but discards a flow unless its C<output> field matches a
value in I<interface-list>.

=item B<discard-unless> B<any>-B<interfaces> I<interface-list>

As above, but discards a flow unless either its C<input> or C<output>
field matches a value in I<interface-list>.
...
These optional commands are similar to the
S<B<discard-unless> I<*>-B<interfaces>> commands above, except they
operate on IP addresses.  That is, the first discards a flow unless
its source IP address is in I<cidr-block-list>, the second discards a
flow unless its destination IP address is in I<cidr-block-list>, and
the third discards a flow unless either the source IP or destination
IP address is in I<cidr-block-list>.
...
B<remainder> or a list containing CIDR blocks and/or group references.
When the value is the keyword B<remainder>, the IP-space for
I<network-name> is computed by finding all IPs not assigned to other
networks on this sensor.  The B<remainder> keyword may only appear one
time within a sensor block.
...
The value of the command can be the keyword B<remainder> or a list
containing interface numbers and/or group references.  When the value
is the keyword B<remainder>, the interface list is computed by finding
all interface values not assigned to other networks on this sensor.
...
categorize the flows, where traffic that enters the router on
interface 1 and leaves on interface 8 is B<in>, traffic entering on 8
and leaving on 1 is B<out>, traffic from 1 to 0 is B<innull>, traffic
from 8 to 8 is B<int2int>, etc.
...
configuration is the same as in the previous example.
...
logical sensor.  The following solution has each stream arrive on a
separate port where it is collected by a separate probe.  The probes
are combined into one sensor.  This solution works with all types of
input; see the next example for a more limited solution.
...
It may be possible to simplify the previous example that combines
multiple input streams to a single sensor depending on the type of the
input streams and on the versions of software being used.  In the more
simple configuration, you send all the input streams to a single port,
and you define a single probe listening on that port.  As long as the
streams have a unique source IP, the streams will be treated
distinctly.  This configuration works for NetFlow v5 on all versions
of SiLK, for TCP-based IPFIX as of SiLK 3.0 and libfixbuf-1.0, and for
NetFlow v9 and UDP-based IPFIX as of SiLK 3.4 and libfixbuf-1.2.0.
(If these requirements are not met, use the configuration in the
previous example.)  The following configurations will accept any
number of TCP-based IPFIX connections to port 9907 and any number of
NetFlow v5 connections to 9908, and treat all the flow data
identically.
...
or B<yaf> processes) that you wish to treat as separate sensors, but
you want to open a single port on your firewall to collect these flow
...
B<listen-on-port> value but different B<accept-from-host> values.  The
following configuration uses a NetFlow v5 probe, which works for all
versions of SiLK.  A similar configuration will work for TCP-based
IPFIX probes as of SiLK 3.0 and libfixbuf-1.0.  The configuration will
also work for UDP-based IPFIX probes or NetFlow v9 probes as of SiLK
3.4 and libfixbuf-1.2.0.
...
particular VLAN tag.  First, specify the B<interface-values> statement
...
statements in each block, you may configure B<rwflowpack> to
...
B<packlogic-generic(3)>, B<silk.conf(5)>, B<silk(7)>,
I<SiLK Installation Handbook>, B<pcap(3)>, B<yaf(1)>, B<gzip(1)>

=head1 NOTES

The B<quirks> statement was introduced in SiLK 3.8.

The following statements were introduced in SiLK 2.1: B<group>,
B<discard-unless>, B<discard-when>, B<interface-values>.

=cut

$SiLK: sensor.conf.pod 37fd860c358b 2014-09-23 13:19:15Z mthomas $
