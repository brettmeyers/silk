...
B<silkpython> - SiLK Python plug-in (silkpython.so)
...
The Python module that the SiLK Python plug-in uses (F<pysilk_nl.so>)
is installed under SiLK's installation tree.  It may be necessary to
set or modify the PYTHONPATH environment variable so Python can find
this module.  For information on using SiLK from Python, see
B<pysilk(3)>.
...
searching for F<silkpython.so>, a SiLK application may use this
...
standard error to assist you in finding problems loading plug-in files
and registering fields.  The application prints messages as it
attempts to find and open the F<silkpython.so> plug-in.  In addition,
...
be rather verbose.  A typical invocation using this variable is
...
=item F<${SILK_PATH}/lib64/silk/silkpython.so>

=item F<${SILK_PATH}/lib64/silkpython.so>

=item F<${SILK_PATH}/lib/silk/silkpython.so>

=item F<${SILK_PATH}/lib/silkpython.so>

=item F<@prefix@/lib64/silk/silkpython.so>

=item F<@prefix@/lib64/silkpython.so>

=item F<@prefix@/lib/silk/silkpython.so>

=item F<@prefix@/lib/silkpython.so>
...
$SiLK: silkpython.pod 477e54df24bf 2014-07-08 18:02:17Z mthomas $
