...
of C<NO_COMPRESSION>, C<ZLIB>, and/or C<LZO1X>.
...
L<silk.B<ipv6_enabled()>|/ipv6_enabled> returns B<True>.  The
...
L<silk.B<init_country_codes()>|/init_country_codes> function.  If
...
Return a tuple of the octets of I<addr>.  This will be a tuple of
length 4 for IPv4 addresses, and a tuple of length 16 for IPv6
addresses.
...
included.  Note that setting or accessing certain attributes on an
B<RWRec> causes the L<silk.site.B<init_site()>|/init_site> function to
be called with no argument if it has not yet been called successfully.
...
(READ ONLY) The class name of assigned to the flow I<rec>, a string.
Calls L<silk.site.B<init_site()>|/init_site>.  The default classname
is C<?>.  The classname cannot be modified by itself.  In order to
modify the classname, you also need to modify the typename.  See the
L<I<rec>.classtype|/rec.classtype> attribute.

=item I<rec>.classtype

A tuple of the classname and the typename of the flow I<rec>.
Implicitly calls L<silk.site.B<init_site()>|/init_site> with no
arguments if L<silk.site.B<have_site_config()>|/have_site_config>
returns B<False>.

=item I<rec>.classtype_id

The ID for the class and type of the flow I<rec>.  The default
classtype_id value is 0.
...
The duration of the flow I<rec> in seconds, a float.  The default
...
etime_epoch_secs value 0.  Changing the I<rec>.etime_epoch_secs
...
I<rec>.protocol is ICMP (1) or when B<is_ipv6()|/is_ipv6> is B<True>
...
epoch time, a float.  Epoch time is 1970-01-01 00:00:00.  The default
stime_epoch_secs value 0.  Changing the I<rec>.stime_epoch_secs
...
(READ ONLY) The type name of the flow I<rec>, a string.  Implicitly
calls L<silk.site.B<init_site()>|/init_site> with no arguments if
L<silk.site.B<have_site_config()>|/have_site_config> returns B<False>.
The default typename is 255.  The typename cannot be modified by
itself.  In order to modify the typename, you also need to modify the
classname.  See the L<I<rec>.classtype|/rec.classtype> attribute.
...
protocol of I<rec> is 58 (ICMPv6) and B<is_ipv6()/is_ipv6> is B<True>.
Return B<False> otherwise.
...
L<silk.B<get_configuration("COMPRESSION_METHODS")>|get_configuration>).
...
described in the L<silk.B<silkfile_open()>|/silkfile_open> function.
...
identical to those used by the L<B<silkfile_open()>|/silkfile_open>
function.  This constructor is deprecated as of SiLK 3.0.0.  For
future compatibility, please use the
L<B<silkfile_open()>|/silkfile_open> function instead of the
...
L<silk.B<ipv6_enabled()>|/ipv6_enabled> is B<False>, in which case the
...
L<silk.B<ipv6_enabled()>|/ipv6_enabled> returns B<False>.  An error
...
L<silk.B<ipv6_enabled()>|/ipv6_enabled> is B<False>).  I<counter_type>
...
L<silk.site.B<repository_iter()>|/repository_iter> function from the
...
file every time, whereas the L<silk.site.B<init_site()>|/init_site>
...
configuration file (see B<silk.conf(3)>).  If I<siteconf> is omitted,
...
L<silk.site.B<have_site_config()>|/have_site_config> will return
...
L<silk.site.B<init_site()>|/init_site> function is implicitly invoked
...
Return B<True> if L<silk.site.B<init_site()>|/init_site> has been
...
L<silk.site.B<init_site()>|/init_site> with a I<rootdir> argument.
B<set_data_rootdir()> implicitly calls
L<silk.site.B<init_site()>|/init_site> with no arguments before
...
L<silk.site.B<init_site()>|/init_site> is called successfully, this
...
L<silk.site.B<init_site()>|/init_site> with no arguments if
L<silk.site.B<have_site_config()>|/have_site_config> returns B<False>.
...
description.  Implicitly calls L<silk.site.B<init_site()>|/init_site>
...
ID I<id>.  Implicitly calls L<silk.site.B<init_site()>|/init_site>
with no arguments if
L<silk.site.B<have_site_config()>|/have_site_config> returns B<False>.
...
returned by L<silk.site.B<get_data_rootdir()>|/get_data_rootdir> and
to conform to the format of the current site configuration.  This
function implicitly calls L<silk.site.B<init_site()>|/init_site> with
no arguments if L<silk.site.B<have_site_config()>|/have_site_config>
returns B<False>.  See also
L<silk.site.B<repository_full_iter()>|/repository_full_iter> and
L<silk.site.B<repository_silkfile_iter()>|/repository_silkfile_iter>.
...
L<silk.B<get_configuration("TIMEZONE_SUPPORT")>|get_configuration>.
...
L<silk.site.B<classes()>|/classes>.  If neither I<classname> nor
I<classtypes> are specified, I<classname> will default to that
returned by L<silk.site.B<default_class()>|/default_class>.
...
Works similarly to L<silk.site.B<repository_iter()>|/repository_iter>
...
L<silk.site.B<init_site()>|/init_site> if no argument is passed to
that method.
...
L<silk.site.B<init_site()>|/init_site> may search for the site
...
L<silk.B<init_country_codes()>|/init_country_codes> function will use
...
which L<silk.site.B<repository_iter()>|/repository_iter> parses
...
L<silk.site.B<init_site()>|/init_site>.
...
L<silk.B<init_country_codes()>|/init_country_codes> when no name is
...
B<python(1)>, B<gzip(1)>, B<yaf(1)>, L<http://docs.python.org/>

=cut

$SiLK: pysilk.pod 7ed42ea5166b 2014-08-08 14:53:18Z mwd $
