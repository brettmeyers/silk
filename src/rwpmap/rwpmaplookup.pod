...
B<--xargs> switch is provided, B<rwpmaplookup> will read the names of
...
specify how the IP addresses will be printed.  When this switch is not
...
Determines where the output of B<rwpmaplookup> is written.  If this
...
the value of the pager is determined to be the empty string, no paging
will be performed and all output will be printed to the terminal.

=item B<--xargs>

=item B<--xargs>=I<FILENAME>

Causes B<rwpmaplookup> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwpmaplookup> will open each file in turn
...
$SiLK: rwpmaplookup.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
