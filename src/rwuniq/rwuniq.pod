...
When the B<--xargs> switch is provided, B<rwuniq> will read the names
...
B<rwuniq> does not need large amounts of memory because it does not
...
B<rwuniq> may run out of memory when computing distinct IP counts,
...
standard error noting that B<rwuniq> has run out of memory, processing
continues, and B<rwuniq> exits with status 16.
...
B<rwuniq> to process an endless stream of records.  When multiple
input files are specified, B<rwuniq> will merge-sort the flow records
from the input files.  See the L</NOTES> section for issues that may
occur when using B<--presorted-input>.
...
B<--output-path> switch has been used to redirect B<rwuniq>'s ASCII
output.

=item B<--output-path>=I<PATH>

Determines where the output of B<rwuniq> (ASCII text) is written.  If
...
Causes B<rwuniq> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwuniq> will open each file in turn and
...
Instruct B<rwuniq> to load the mapping file located at I<PATH> and
...
specific to that prefix map.  If I<MAPNAME> is not provided, B<rwuniq>
will check the prefix map file to see if a map-name was specified when
the file was created.  If no map-name is available, B<rwuniq> creates
...
B<--python-file> is specified, B<rwuniq> loads Python which in turn
...
the output from B<rwuniq --version>), the value of the TZ environment
variable determines the timezone in which B<rwuniq> displays
...
Bag tools usually provide better performance, especially when the key
is one or two bytes:
...
B<rwfilter(1)>, B<rwbag(1)>, B<rwcut(1)>, B<rwset(1)>, B<rwsetcat(1)>,
B<rwaddrcount(1)>, B<rwgroup(1)>, B<rwstats(1)>, B<rwnetmask(1)>,
B<rwsort(1)>, B<rwtotal(1)>, B<rwcount(1)>, B<addrtype(3)>,
B<ccfilter(3)>, B<pmapfilter(3)>, B<pysilk(3)>, B<silkpython(3)>,
B<silk-plugin(3)>, B<sensor.conf(5)>, B<rwflowpack(8)>, B<silk(7)>,
B<yaf(1)>, B<dlopen(3)>

=cut

$SiLK: rwuniq.pod 477e54df24bf 2014-07-08 18:02:17Z mthomas $
