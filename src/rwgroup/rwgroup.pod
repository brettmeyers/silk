...
The first group of records gets the identifer 0, and B<rwgroup> writes
that value into each record's Next Hop IP field.  The ID for each
subsequent group is incremented by 1.  The B<--group-offset> switch
may be used to set the identifier of the initial group.
...
B<--output-path> switch has been used to redirect B<rwgroup>'s output.

=item B<--output-path>=I<PATH>

Determines where the output of B<rwgroup> is written.  If
...
Instruct B<rwgroup> to load the mapping file located at I<PATH> and
...
B<rwgroup> will check the prefix map file to see if a map-name was
specified when the file was created.  If no map-name is available,
B<rwgroup> creates the fields sval and dval.  Multiple B<--pmap-file>
switches are supported as long as each uses a unique value for
map-name.  The B<--pmap-file> switch(es) must precede the B<--id-fields>
...
B<--python-file> is specified, B<rwgroup> loads Python which in turn
...
B<silkpython(3)>, B<silk-plugin(3)>, B<sensor.conf(5)>, B<uniq(1)>,
B<silk(7)>, B<yaf(1)>, B<dlopen(3)>, B<zlib(3)>

=cut

$SiLK: rwgroup.pod 675af7f47d1f 2014-09-24 21:57:48Z mthomas $
