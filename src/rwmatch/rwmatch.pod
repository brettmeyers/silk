...
address into the ::ffff:0:0/96 prefix.
...
The F<cutmatch.so> plug-in is an alternate way to display the match
parameter that B<rwmatch> writes into the next hop IP field.  The
F<cutmatch.so> plug-in defines a C<match> field that displays the
...
B<rwscan(1)>, B<rwscanquery(1)>, B<sensor.conf(5)>, B<silk(7)>,
B<zlib(3)>
...
$SiLK: rwmatch.pod b0322a07081d 2014-09-24 18:55:26Z mthomas $
