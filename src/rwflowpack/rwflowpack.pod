...
Processing of IPFIX and NetFlow v9 is only available when SiLK is
compiled with support for libfixbuf-1.3.0 or later.  Processing of
sFlow records was added in SiLK 3.9.0 and requires libfixbuf-1.6.0 or
later.  libfixbuf is available from
...
B<rwflowpack> will not become a daemon in this input-mode; instead it
...
required, and their use will cause B<rwflowpack> to exit with an error
code.
...
The B<rwflowappend> process can poll the incremental-directory itself,
or another process can poll that directory and pass the files to
B<rwflowappend>.  If B<rwflowpack> and B<rwflowappend> are on
different machines, an B<rwsender(8)>/B<rwreceiver(8)> pair can move
the files between the machines.
...
Determine how B<rwflowpack> will gather data.  The default input
I<MODE> is C<stream>.  The available input-modes are C<stream>,
C<fcfiles>, C<pdufile>, and C<respool>.

=item B<--output-mode>=I<MODE>

Determines what B<rwflowpack> will do with the data as it is packed
into SiLK binary files.  The default output I<MODE> is
C<local-storage>.  The available output-modes are C<local-storage>,
C<incremental-files> and C<sending>.
...
slash (C</>), B<rwflowpack> will attempt to find a file named
I<PLUGIN> in the directories listed in the L</FILES> section.  If
B<rwflowpack> finds the file, it uses that path.  If I<PLUGIN>
contains a slash or if B<rwflowpack> does not find the file,
B<rwflowpack> relies on your operating system's B<dlopen(3)> call to
find the file.  When the SILK_PLUGIN_DEBUG environment variable is
non-empty, B<rwflowpack> prints status messages to the standard error
as it attempts to find and open each of its plug-ins.  B<rwflowpack>
will exit if it does not have access to the packing logic functions.

=item B<--sensor-name>=I<SENSOR>

Cause B<rwflowpack> to ignore all probes in the sensor configuration
file except the probes for I<SENSOR>.  Only data for this sensor will
be collected.  This allows a common configuration file to be used by
...
switch is not present, B<rwflowpack> will collect and pack data for
all sensors.

=item B<--polling-interval>=I<NUMBER>

Specify the number of seconds B<rwflowpack> will wait between queries
of the C<poll-directory>s.  This defaults to 15 seconds.
...
Give the path to the configuration file that B<rwflowpack> will
...
described in the B<sensor.conf(5)> manual page; see also the I<SiLK
Installation Handbook>.

=item B<--incoming-directory>=I<DIR_PATH>

Name the full path of the directory which B<rwflowpack> will monitor
for files created by B<flowcap>.  Once processed by B<rwflowpack>,
files are moved from this directory to the archive-directory, if it
has been specified.
...
Specify the number of seconds B<rwflowpack> will wait between polls of
...
Name the full path of the directory which B<rwflowpack> will monitor
for SiLK Flow files.  Once processed by B<rwflowpack>, files are moved
...
under which the files containing the packed SiLK Flow records will be
stored.  B<rwflowpack> will create subdirectories below I<DIR_PATH>
...
switch can be use when writing data to these file systems.
...
files B<rwflowpack> will read from simultaneously when using probes
...
Specify how to compress newly created files.  When this switch is not
...
with the full path to the input file in the archive-directory, and
each occurrence of C<%%> will be replaced with C<%>.  If any other
character follows C<%>, B<rwflowpack> exits with an error.  When using
this feature, the B<--archive-directory> switch must be specified.
See also the B<rwpollexec(8)> daemon.
...
B<rwflowpack> exits with an error.  Specifying this switch without
...
$SiLK: rwflowpack.pod 37fd860c358b 2014-09-23 13:19:15Z mthomas $
