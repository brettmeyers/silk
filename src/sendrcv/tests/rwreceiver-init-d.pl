...
#
#
# RCSIDENT("$SiLK: rwreceiver-init-d.pl 7db00ddfdd25 2014-05-30 18:58:37Z mthomas $")
...
# assume an rwsender is listening on this port and host
my $host = '127.0.0.1';
...
while (<SRC>) {
    ...
    if (/^(POST_COMMAND=).*/) {
        print CONF $1, "'sender \%I sent file \%s'", "\n";
        next;
    }
    if (/^(DUPLICATE_DIRS=)/) {
        $skip = 1;
        print CONF $_,"\n";
        print CONF "  $tmpdir/duplicate-dir1\n";
        print CONF "  $tmpdir/duplicate-dir2\n";
        next;
    }
    if (/^(SENDER_SERVERS=)/) {
        $skip = 1;
        print CONF $_,"\n";
        print CONF "  sender-1  $host:$port\n";
        next;
    }
    if ($skip) {
        if (/^END_DUPLICATE_DIRS/) {
            print CONF $_,"\n";
            $skip = 0;
        }
        if (/^END_SENDERS/) {
