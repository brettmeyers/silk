...
B<rwcut> reads binary SiLK Flow records and prints the records to the
screen in a textual, bar (C<|>) delimited format.  See the EXAMPLES
...
When the B<--xargs> switch is provided, B<rwcut> will read the names
...
the ICMP code value for ICMP or ICMPv6 flows and emtpy for non-ICMP
...
I<FORMAT> is a comma-separated list of a format, a timezone, and/or
a modifier.  The format is one of:
...
B<--output-path> switch has been used to redirect B<rwcut>'s ASCII
output.

=item B<--output-path>=I<PATH>

Determines where the output of B<rwcut> (ASCII text) is written.  If
...
Causes B<rwcut> to read file names from I<FILENAME> or from the
standard input if I<FILENAME> is not provided.  The input should have
one file name per line.  B<rwcut> will open each file in turn and read
...
Instruct B<rwcut> to load the mapping file located at I<PATH> and
...
specific to that prefix map.  If I<MAPNAME> is not provided, B<rwcut>
will check the prefix map file to see if a map-name was specified when
the file was created.  If no map-name is available, B<rwcut> creates
...
B<--python-file> is specified, B<rwcut> loads Python which in turn
...
the output from B<rwcut --version>), the value of the TZ environment
variable determines the timezone in which B<rwcut> displays
...
B<rwfilter(1)>, B<num2dot(1)>, B<addrtype(3)>, B<ccfilter(3)>,
B<pmapfilter(3)>, B<silk-plugin(3)>, B<silkpython(3)>, B<pysilk(3)>,
B<sensor.conf(5)>, B<silk(7)>, B<yaf(1)>, B<dlopen(3)>

=cut

$SiLK: rwcut.pod 477e54df24bf 2014-07-08 18:02:17Z mthomas $
