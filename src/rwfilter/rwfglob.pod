...
Only a single class may be selected.  Classes are defined in the
B<silk.conf(5)> site configuration file.  If the B<--class> option is
not given, the default-class as specified in F<silk.conf> is used.
Use B<rwfglob --help> to see the list of available classes and the
default class.
...
list of default types will be used.  The default-type list is
determined by the value of I<CLASS>, and the default types generally
include only incoming traffic.  Use B<rwfglob --help> to see the
...
class/type pairs.  The B<--flowtype> switch allows a single B<rwfglob>
invocation to print data from multiple classes.  The keyword C<all>
may be used for the I<CLASS> and/or I<TYPE> to select all classes
and/or types.
...
This option prints to the standard error file names that B<rwfglob>
expected to find but did not.  This switch is useful for debugging,
but the list of files it produces can be misleading.  For example,
suppose there is a decommissioned sensor that still appears in the
F<silk.conf> file to permit retrieval of historical data; these data
files will be I<missing> even though their absence is expected.  Use
the output from this switch judiciously.  The file names are preceded
...
$SiLK: rwfglob.pod cd09f0a5fcc6 2014-06-14 15:43:33Z mthomas $
