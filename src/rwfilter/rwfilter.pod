...
used.  Use B<rwfilter --help> to see the list of available classes
...
The B<--flowtype> predicate provides an alternate way to specify
...
B<--start-date>, or to specify B<--start-date> with the
L</Non-Selection Input Switches>.
...
list of default types is used.  The default-type list is
determined by the value of I<CLASS>, and the default types generally
include only incoming traffic.  Use B<rwfilter --help> to see the
list of available types and the default types for each class.
...
Tell B<rwfilter> to read file names from I<INPUT_PATH>; if
I<INPUT_PATH> is not provided, the names of the files are read from
the standard input.  The input should have one file name per line.
B<rwfilter> opens each file in turn and read records from it.
...
its argument an I<IP_WILDCARD>.  An I<IP_WILDCARD> is a single IP
address, a single CIDR block, or a single SiLK IP Wildcard.  A SiLK IP
Wildcard can represent multiple IPv4 or IPv6 addresses.  An IP
...
with B<rwsetbuild(1)>.  Currently, IPsets only support IPv4 addresses.
...
fifteen minutes of Jan 31, 2003.  (You may use a C<T> in place of C<:>
to separate the day and hour; you may also specify the times as
seconds since the UNIX epoch.)  The start-window and end-window must
...
from the output of B<rwfilter --version>.
...
the first packet in the flow, and the second contains the bitwise OR
...
when looking at the bitwise OR of the TCP flags across B<all> packets
in the flow.
...
As B<--flags-all>, except this switch considers the bitwise OR of the
...
Instruct B<rwfilter> to load the mapping file located at I<PATH> and
create new switches B<--pmap-src-I<MAPNAME>>,
B<--pmap-dst-I<MAPNAME>>, and B<--pmap-any-I<MAPNAME>>.  When
I<MAPNAME> is provided, it is used to refer to the switches
specific to that prefix map.  If I<MAPNAME> is not provided,
B<rwfilter> checks the prefix map file to see if a map-name was
specified when the file was created.  If no map-name is available,
B<rwfilter> creates legacy switches as described below.  Multiple
B<--pmap-file> switches are supported as long as each uses a unique
map-name.  The B<--pmap-file> switch(es) must precede all other
...
If the prefix map associated with I<MAPNAME> is an IP prefix map, this
matches records with a source IP address or a destination IP address
that maps to a label contained in the list of labels in I<LABELS>.

If the prefix map associated with I<MAPNAME> is a port/protocol prefix
...
to B<--pmap-src-I<MAPNAME>>, B<--pmap-dst-I<MAPNAME>>, and
B<--pmap-any-I<MAPNAME>>, respectively.  These switches are available
when an IP prefix map is used that is not associated with a
I<MAPNAME>.
...
L<http://www.iana.org/cctld/cctld-whois.htm>)---as well as the
...
The SiLK Python plug-in, F<silkpython.so>, provides support for
...
B<--python-file> or B<--python-expr> is specified, B<rwfilter> loads
Python which in turn loads the PySiLK module which is comprised of
several files (F<silk/pysilk_nl.so>, F<silk/__init__.py>, etc).  If
...
module.  For information on using Python from within B<rwfilter>, see
B<pysilk(3)>.
...
B<rwtuc(1)>, B<rwsetbuild(1)>, B<rwsiteinfo(1)>, B<addrtype(3)>,
B<ccfilter(3)>, B<flowrate(3)>, B<ipafilter(3)>, B<pmapfilter(3)>,
B<pysilk(3)>, B<silkpython(3)>, B<silk-plugin(3)>, B<silk.conf(5)>,
B<sensor.conf(5)>, B<silk(7)>, B<rwflowpack(8)>, B<yaf(1)>,
B<applabel(1)>, B<zlib(3)>, B<dlopen(3)>, I<Analysts' Handbook: Using
SiLK for Network Traffic Analysis>

=cut

$SiLK: rwfilter.pod edfa31676092 2014-07-08 18:22:52Z mthomas $
