...
RCSIDENT("$SiLK: rwfiltersetup.c b7b8edebba12 2015-01-05 18:05:21Z mthomas $");
    ...
    char               *opt_arg)
{
    ...
    switch ((appOptionsEnum)opt_index) {
      ...
      case OPT_ALL_DEST:
        ...
        if ((rv = skStreamCreate(&dest->ios, SK_IO_WRITE,SK_CONTENT_SILK_FLOW))
            || (rv = skStreamBind(dest->ios, opt_arg)))
        {
    ...
    void)
{
    ...
    for (dest_id = 0; dest_id < DESTINATION_TYPES; ++dest_id) {
        ...
        {
            ++count;
            if (strcmp("stderr", skStreamGetPathname(dest->ios)) == 0) {
                skAppPrintErr(("Invalid %s '%s': Will not write"
                               " binary data to the standard error"),
                              appOptions[dest_id+OPT_PASS_DEST].name,
                              skStreamGetPathname(dest->ios));
                return -1;
            }
            if ((strcmp("stdout", skStreamGetPathname(dest->ios)) == 0)
                || (strcmp("-", skStreamGetPathname(dest->ios)) == 0))
            {
                if (stdout_used) {
                    skAppPrintErr("Invalid %s '%s':"
    ...
    void)
{
    ...
    for (dest_id = 0; dest_id < DESTINATION_TYPES; ++dest_id) {
             ...
             dest = dest->next)
        {
            ...
            rv = skHeaderSetCompressionMethod(skStreamGetSilkHeader(dest->ios),
                                              comp_method);
            if (rv) {
                goto DEST_ERROR;
            }
            rv = skStreamOpen(dest->ios);
            if (rv) {
                goto DEST_ERROR;
            }
        }
